...
describe("function get_position_ids", () => {
  test("it can get the position ids of an address", async () => {
    expect(
      await utils.get_position_ids("0xe403043a0f9c7b9f315cf145166eb747d9790e77")
    ...
    expect(
      ...
      expect.arrayContaining([734999n, 734427n, 664870n, 654853n, 619539n])
    );
  });

  test("it returns an empty array when it cannot get any position ids of an address", async () => {
    expect(
      await utils.get_position_ids("0x7b1E5D984A43eE732de195628d20d05CFaBc3cC7")
    ).toEqual([]);
  });
});
