...
async function calculate_lp(pool_address, user_address) {
  ...
  if (
    ...
    try {
      const is_pool_address_valid = await validations.is_pool_address_valid(
        pool_address
      );
      ...
      if (is_pool_address_valid) {
        ...
        const token_symbols = await utils.get_token_symbols(token_0, token_1);
        ...
        const unclaimed_token_fees = await utils.get_unclaimed_token_fees(
          position.id,
          user_address,
          token_0,
          token_1
