...
async function get_token_values(lp_address, token_0_address, token_1_address) {
  const token_0_contract = new ethers.Contract(token_0_address, constants.ERC_20_ABI, constants.MAINNET_PROVIDER);
  const token_1_contract = new ethers.Contract(token_1_address, constants.ERC_20_ABI, constants.MAINNET_PROVIDER);

  const token_0_balance = await token_0_contract.balanceOf(lp_address);
  const token_1_balance = await token_1_contract.balanceOf(lp_address);
...
async function get_token_fees(lp_address) {
  const contract = new ethers.Contract(lp_address, constants.UNISWAP_V3_POOL_ABI, constants.MAINNET_PROVIDER);
  const [token_0_fee, token_1_fee] = await contract.protocolFees();
...
async function get_token_amounts(lp_address, token_0_address, token_1_address) {
  const token_0_contract = new ethers.Contract(token_0_address, constants.ERC_20_ABI, constants.MAINNET_PROVIDER);
  const token_1_contract = new ethers.Contract(token_1_address, constants.ERC_20_ABI, constants.MAINNET_PROVIDER);

  const token_0_amount = await token_0_contract.balanceOf(lp_address);
  const token_1_amount = await token_1_contract.balanceOf(lp_address);
