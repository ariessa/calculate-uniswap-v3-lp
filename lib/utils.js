...
const { mainnet_provider, testnet_provider } = require("../constants/providers");
...
async function get_contract_creation_block() {
    ...
    const response = await fetch(`${env.ETHERSCAN_MAINNET_URL}?module=contract&action=getcontractcreation&contractaddresses=${env.UNISWAP_V3_FACTORY_ADDRESS}&apikey=${env.ETHERSCAN_API_KEY}`);
    const data = await response.json();
    const tx_hash = await data.result[0].txHash;

    // Get the transaction receipt for the transaction hash
    const receipt = await mainnet_provider.getTransactionReceipt(tx_hash);
...
async function get_last_finalised_block() {
    ...
    const latestFinalizedBlockNumber = await mainnet_provider.getBlockNumber();
...
async function get_last_processed_block() {
    ...
    const contract = new ethers.Contract(env.CONTRACT_ADDRESS, contractAbi, testnet_provider);
...
async function add_liquidity_pool(lp_address, token_a, token_b, fee, block_number) {
    ...
    const wallet = new ethers.Wallet(env.WALLET_TESTNET_PRIVATE_KEY, testnet_provider);
